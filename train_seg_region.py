...
from pytorch_grad_cam.utils.image import normalize
...
def main(_run, _config, _log):
    ...
    CAM_Region = GradCAM(model=model, target_layers=[model.backbone.layer4[-1] ], use_cuda=True)
    model.train()
    ...
    for epoch in range(opt.epoch_count):
        ...
        for i, train_batch in enumerate(train_loader):
            ...
            for c in range(opt.num_classes-1):
                targets = [SemanticSegmentationTarget(c+1, lb)]
                cam = CAM_Region(input_tensor=img, targets=targets) 
                cam = torch.maximum(cam, torch.tensor(0)) # ReLu X 与 Y 逐位比较取其大者
                region_cam = cam.sum(dim=1, keepdim=True)
                region_cam_norm = normalize(region_cam) # 归一化 [20, 1, 12, 12]
                region_mask = region_cam_norm.ge(0.5).float()

                group_region_feature = torch.empty(size=(feat_dim, 0)).cuda()

                for bs in range(opt.batchsize):
                    this_feat = f[bs].contiguous().view(feat_dim, -1)
                    this_label = region_mask[bs].contiguous().view(-1)
                    idxs = this_label.nonzero()
                    region_feature = this_feat[:, idxs].squeeze(2)
                    region_feature = nn.functional.normalize(region_feature, p=2, dim=0)
                    group_region_feature = torch.cat((group_region_feature, region_feature), dim=1)
                
                feat_mu = torch.mean(group_region_feature, dim=1)  # 均值
                feat_sig = torch.std(group_region_feature, dim=1) # 方差

                print(feat_mu.shape, feat_sig.shape)
