...
from dataloaders.normalize_utils import get_normalize_op
...
# folder for datasets
BASEDIR = '/mnt8t/lijiaxi/Datasets/Mms'
print(f'Running on machine {hostname}, using dataset from {BASEDIR}')
LABEL_NAME = ["lv", "myo", "rv", "bg"]


class MmsDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains:list,  idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, extern_norm_fn = None):
        ...
        if extern_norm_fn is None:
            self.normalize_op = get_normalize_op('MR', [itm['img_fid'] for _, itm in self.sample_list[self.domains[0]].items()])
    ...
    def __read_dataset(self):
        ...
        for _domain, _sample_list in self.sample_list.items():
            for scan_id, itm in _sample_list.items():
                ...
                img = np.float32(img).reshape(-1, img.shape[2], img.shape[3])

                lb = nio.read_nii_bysitk(itm["lbs_fid"])
                lb = np.float32(lb).reshape(-1, lb.shape[2], lb.shape[3])
                ...
                # Keep only ES and ED
                ES_ED = np.load(self._base_dir + '/' + _domain + '.npy', allow_pickle=True).item()
                scan_id_z = ES_ED[scan_id]
                if len(scan_id_z) == 0:
                    continue

                # now start writing everthing in
                # write the beginning frame
                out_list.append( {"img": img[..., scan_id_z[0]: scan_id_z[0] + 1],
                               "lb":lb[..., scan_id_z[0]: scan_id_z[0] + 1],
                               "is_start": True,
                               "is_end": False,
                               "domain": _domain,
                               "nframe": len(scan_id_z),
                               "scan_id": _domain + "_" + scan_id,
                               "z_id":0})
                glb_idx += 1

                for ii in range(1, len(scan_id_z) - 1):
                    out_list.append( {"img": img[..., scan_id_z[ii]: scan_id_z[ii] + 1],
                               "lb":lb[..., scan_id_z[ii]: scan_id_z[ii] + 1],
                               "is_start": False,
                               "is_end": False,
                               "nframe": -1,
                               "domain": _domain,
                ...
                out_list.append( {"img": img[..., scan_id_z[-1]: scan_id_z[-1] + 1],
                               "lb":lb[..., scan_id_z[-1]: scan_id_z[-1] + 1],
                               "is_start": False,
                               "is_end": True,
                               "nframe": -1,
                               "domain": _domain,
    ...
    def __getitem__(self, index):
        ...
        re = self.info_by_scan[curr_dict["scan_id"]]['spacing'][0]
        ...
        region = lb[0].clone()
        non_zero_index = torch.nonzero(region, as_tuple=False)
        if region.sum() > 0:
            ...
            x_region = lb.size(-2) // 2
            y_region = lb.size(-1) // 2

        # Resize & Pad to new size
        resize_order = re / 1.1
        resize_size_h = int(img.size(-2) * resize_order)
        resize_size_w = int(img.size(-1) * resize_order)
        ...
        if top_size >= 0:
            ...
            if x_region > self.new_size//2:
                if x_region - self.new_size//2 + self.new_size <= lb.size(-2):
                    top_crop = x_region - self.new_size//2
                else:
                    top_crop = lb.size(-2) - self.new_size
            else:
                top_crop = 0
        ...
        if left_size >= 0:
            ...
            if y_region > self.new_size//2:
                if y_region - self.new_size//2 + self.new_size <= lb.size(-1):
                    left_crop = y_region - self.new_size//2
                else:
                    left_crop = lb.size(-1) - self.new_size
            else:
                left_crop = 0
...
def get_training(modality, idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3):
    return MmsDataset(idx_pct = idx_pct,\
        mode = 'train',\
        domains = modality,\
        transforms = tr_func,\
        base_dir =  BASEDIR,\
...
# dataset = MmsDataset(mode='train', transforms=tr_func, base_dir=BASEDIR, domains=['vendorA','vendorB'])
# test_loader = DataLoader(dataset = dataset, num_workers = 1,\
...
#             'lb': batch['lb']
#             }
#     final_image = to01(test_input['img'])[0,0].cpu().numpy()
#     Image.fromarray(final_image.astype(np.uint8)).save(os.path.join('./vendorA', batch['scan_id'][0] + '_' + str(batch['z_id'].numpy()) + '.png'))
