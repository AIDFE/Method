...
import transform_utils as trans
import torch
import os
import platform
...
from monai import transforms as lb_transforms
...
# folder for datasets
BASEDIR = '/mnt/hard_disk/lijiaxi/Datasets/BraTs2018'
print(f'Running on machine {hostname}, using dataset from {BASEDIR}')
LABEL_NAME = ["tc", "wt", "et"]

class BraTsDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains: list,  idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, extern_norm_fn = None):
        ...
        self.lb_transforms = lb_transforms.Compose(lb_transforms.ConvertToMultiChannelBasedOnBratsClassesD(keys=["label"]),
                                                lb_transforms.CropForegroundd(keys=["image", "label"], source_key="image"))
    ...
    def __getitem__(self, index):
        ...
        data = self.lb_transforms({"image":img, "label":lb})
        lb = data["label"][...,0]

        img = np.transpose(img, (2, 0, 1))
        # lb  = np.transpose(lb, (2, 0, 1)) # (240, 240, 1) , (3, 240, 240, 1)
...
def get_training(modality, idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3):
    return BraTsDataset(idx_pct = idx_pct,\
        mode = 'train',\
        domains = modality,\
        transforms = tr_func,\
        base_dir =  BASEDIR,\
...
# dataset = BraTsDataset(mode='train', transforms=tr_func, base_dir=BASEDIR, domains=['T2'])
...
#             'lb': batch['lb']
#             }
#     seg = test_input['lb'].long().sum(dim=1).cpu().numpy()
#     final_image = overlay_seg_img(to01(test_input['img'])[0,0].cpu().numpy(), seg[0])

    
#     Image.fromarray(final_image.astype(np.uint8)).save(os.path.join('./T2', batch['scan_id'][0] + '_' + str(batch['z_id'].numpy()) + '.png'))